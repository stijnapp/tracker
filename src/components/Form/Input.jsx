export default function Input({ type, label, value, setValue, className = "", ...props }) {
    // TODO: add custom classes like floating-label, floating-label-input
    return (
        <label className="relative">
            <input
                ...
                {...props}
                className={`${className} ${type === 'color' && 'h-12'} w-full p-2 rounded-md transition-[box-shadow] duration-200 bg-transparent placeholder-transparent outline-none border border-gray-400 dark:border-gray-500 focus:border-primary focus:ring-1 focus:ring-primary peer`}
            />
