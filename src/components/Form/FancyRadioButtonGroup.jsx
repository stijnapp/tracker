                ...
                {options.map(({ value: optionValue, label: optionLabel, icon }) => (
                    <li key={optionValue} >
                        <label htmlFor={optionValue} className="flex justify-between gap-4 items-center p-2 rounded-lg ring-1 ring-transparent has-[:checked]:ring-primary has-[:checked]:text-primary has-[:checked]:bg-primary/10">
                            ...
                            <input type="radio" name={label} id={optionValue} value={optionValue} checked={value === optionValue} {...props} className="box-content h-2 w-2 appearance-none rounded-full border-[5px] border-floating-light dark:border-floating-dark bg-floating-light dark:bg-floating-dark bg-clip-padding outline-none ring-1 ring-black/20 dark:ring-white/10 checked:border-primary checked:ring-primary" />
                        </label>
                    </li>
                ))}
            </ul>
